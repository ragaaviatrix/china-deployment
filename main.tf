resource "azurerm_public_ip" "transit_gateway_ips" {
  for_each = local.all_gateway_ips
...
resource "azurerm_network_security_rule" "all_transit_gateway_ips" {
  ...
  source_address_prefixes     = local.all_gateway_ips_list
...
# UDP 5000 rule
resource "azurerm_network_security_rule" "udp_5000_rules" {
  name                        = "udp5000_all_gateways"
  priority                    = 2000
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "Udp"
...
# UDP 31283 rule
resource "azurerm_network_security_rule" "udp_31283_rules" {
  name                        = "udp31283_all_gateways"
  priority                    = 3000
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "Udp"
